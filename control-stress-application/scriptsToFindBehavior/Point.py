# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 11:04:10 2013
...
def kmeans(points, k, cutoff):
    ...
    return clusters
def getMinDistToHalfIntervalPoint(cluster,point):
    minDist=999999    
    for c in cluster.points:
        p1=Point(c.coords[0:(len(point.coords))])
        if (getDistance(p1,point)<minDist):
            minDist =getDistance(p1,point)
    return minDist
...
def makeRandomPoint(n, lower, upper):
    ...
    return Point(coords)
def makePointOfList(n, myList):
    coords = []
    for i in range(n): coords.append(float(myList[i]))
    return Point(coords)
# -- Main function
def main(args):
    num_points, n, k, cutoff, lower, upper = 10, 3, 3, 0.5, -200, 200
